...
class WorkOrderService
{
    ...
    public function __construct(WorkOrderRepo $workOrders, TicketRepo $tickets)
    {
    ...
    public function complete(int $workOrderId, array $user, array $payload): array
    {
        ...
        $this->workOrders->updateLaborAndResult($workOrderId, $labor, json_encode($resultPayload), Enums::WORK_ORDER_STATUS_PENDING_QA);
    ...
    public function acceptance(int $workOrderId, array $payload, array $user): array
    {
        ...
        if ($status === Enums::WORK_ORDER_STATUS_IN_PROGRESS) {
            ...
            $this->workOrders->updateLaborAndResult($workOrderId, (int)$workOrder['labor_minutes'], json_encode($existing), $status);
        } else {
            $this->workOrders->updateAcceptance($workOrderId, $status, json_encode($existing));
        }
        return $this->workOrders->findById($workOrderId);
    }
