...
class Http
{
    ...
    public static function error(string $message, int $status = 400, ?string $code = null, ?array $details = null): void
    {
        http_response_code($status);
        header('Content-Type: application/json');
        $payload = [
...
class Util
{
    public static function requestBody(): array
    {
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            Http::error('Invalid JSON payload', 400, 'invalid_json', [json_last_error_msg()]);
            exit;
        }
