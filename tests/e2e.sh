...
SQL
mysql "${MYSQL_URI:-app_db}" < "$TMP_SQL"
...
# Utility: convert datetime
now_iso() { date -u +"%Y-%m-%dT%H:%M:%S"; }
future_hours() { date -u -v +"$1"H +"%Y-%m-%dT%H:%M:%S"; }

# 3. Create project success
info "Create project success"
START=$(future_hours 0)
DUE=$(future_hours 1)
curl -sS -b "$COOKIE_JAR" \
...
PROJECT_ID=$(mysql -N -e "SELECT id FROM projects ORDER BY id DESC LIMIT 1")
echo "Project ID: $PROJECT_ID"

# Create project with invalid time
info "Create project invalid time"
START=$(future_hours 2)
DUE=$(future_hours 1)
curl -sS -b "$COOKIE_JAR" \
  -X POST "$BASE_URL/projects/create" \
...
DEVICE_ID=$(mysql -N -e "SELECT id FROM devices WHERE code='DEV-001'")
...
CHECKOUT_ID=$(mysql -N -e "SELECT id FROM checkouts ORDER BY id DESC LIMIT 1")
...
mysql -e "SELECT id, action, entity_type, created_at FROM audit_logs ORDER BY id DESC LIMIT 10"
mysql -e "SELECT id, status FROM devices WHERE id = $DEVICE_ID"
mysql -e "SELECT id, title, body, created_at FROM notifications ORDER BY id DESC LIMIT 10"
